...
public class cBaseDatos {
    ...
    public boolean logear( String email, String password ) throws SQLException {
        String sql = "SELECT * FROM t_usuarios WHERE nombre=? AND clave=?";
        try {
            Connection db = this.Conectar();
            PreparedStatement consulta = db.prepareStatement(sql); 
            consulta.setString(1, email);
            consulta.setString(2, password);

            try (ResultSet datos = consulta.executeQuery()) {
